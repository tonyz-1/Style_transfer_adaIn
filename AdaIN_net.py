...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            content_feature = self.encode(content)[3]
            t = self.adain(content_feature, style_features[-1])
            t = (1 - alpha) * content_feature + alpha * t
            g = self.decode(t)
            f = self.encode(g)

            loss_c = self.content_loss(f[-1], t)
            loss_s = self.style_loss(f[0], style_features[0])
            for i in range(1, 4):
                loss_s += self.style_loss(f[i], style_features[i])
            ...
            style_features = self.encode(style)
            content_feature = self.encode(content)[3]
            feat = self.adain(content_feature, style_features[-1])
            feat = (1 - alpha) * content_feature + alpha * feat
